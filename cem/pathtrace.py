...
FAKE_IP_REFUSAL = (
    ...
    "所以这里不测。\n"
    "\n"
    "受影响的只有「路径质量」这一项。出口 IP、落地地区、边缘机房、"
    "四段延迟全都照常准 —— 那几项是对域名建一条真的 HTTPS 连接、"
    "问目的地「你看到的我是谁」拿到的；连接照样会被分流器转发出去，"
    "目的地那侧看到的就是你真实的出口。占位地址只是本机内部的一个标签，"
    "拦不住这条问答。\n"
    "拦得住的是 ICMP 逐跳探测：它必须往一个具体的 IP 发包，而这个 IP 是假的。\n"
    "\n"
    "想把逐跳也量出来，三选一：\n"
    "  A. 在分流器里给这个域名配 real-ip（或关掉 fake-ip），让它解析到真实地址\n"
    "  B. 直接对你的代理节点地址跑 mtr —— 那才是本机能看见的那一段\n"
    "  C. 用「延迟分解」里的 TCP / TLS 代替：那是真实业务流量量出来的，"
    "不受 fake-ip 影响，只是给不出逐跳明细"
)
